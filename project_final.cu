...
#define AGGR_BLOCK_SIZE 256
#define NODE_NUM 2708
//#define NODE_NUM 3312 //for citeseer
// #define NODE_NUM 232965 //for reddit
...
__global__ void aggregation_gpu(int *graph_c_indexes, int *graph_c_neighbours, float *in_feature_data, float *out_feature_data, int feature_num_in, int node_num_in){
 ...
 __global__ void aggregation_gpu_tile(int *graph_c_indexes, int *graph_c_neighbours, float *in_feature_data, float *out_feature_data, int feature_num_in, int node_num_in){	
    ...
    int feature_num = feature_num_in;
	int node_num = node_num_in;
  __shared__ float in_feature_tile[NODE_NUM];

		for(int i=0 ; i<node_num; i+=blockDim.x){	
 			if((i+tx) < node_num)
				in_feature_tile[i+tx] = in_feature_data[bx * node_num + i+tx];
		}
  __syncthreads ();
    
	if(bx < feature_num) {
		if(th_per_thread < node_num){
			for (int j = graph_c_indexes[th_per_thread]; j < graph_c_indexes[th_per_thread + 1]; ++j) {
				out_feature_data[bx * node_num + th_per_thread] += in_feature_tile[graph_c_neighbours[j]];
			}
			out_feature_data[bx * node_num + th_per_thread] /= ((float)graph_c_indexes[th_per_thread + 1] - graph_c_indexes[th_per_thread]);
		}
	}
	__syncthreads();
 }
...
__global__ void aggregation_gpu_block(int *graph_c_indexes, int *graph_c_neighbours, float *in_feature_data, float *out_feature_data, int feature_num_in, int node_num_in){
    ...
    if(th_per_thread == 0 && blockIdx.x == 0)
		printf("in gpu gdx = %d, gdy = %d\n", gridDim.x, gridDim.y);
	if(bx < feature_num) {
		if(th_per_thread < node_num){
			for (int j = graph_c_indexes[th_per_thread]; j < graph_c_indexes[th_per_thread + 1]; ++j) {
				out_feature_data[bx * node_num + th_per_thread] += in_feature_data[bx * node_num + graph_c_neighbours[j]];
			}
			out_feature_data[bx * node_num + th_per_thread] /= ((float)graph_c_indexes[th_per_thread + 1] - graph_c_indexes[th_per_thread]);
		}
	}
...
int main(int argc, char const *argv[]) {
	...
	struct timeval stop_all, start_all;
	...
	feature_c.feature_num = feature_num;
        ...
        struct timeval stop_agg, start_agg;
	gettimeofday(&start_agg, NULL); //first aggregation start
...
//	aggregation_gpu_block<<<gd, bd>>>(device_graph_indexes, device_graph_neighbours, device_feature_cata_in, device_feature_cata_out, feature_num, node_num);
	...
	cudaDeviceSynchronize();
        ...
        cudaDeviceSynchronize();
        gettimeofday(&stop_agg, NULL);
	float secs1_agg = (double)(stop_agg.tv_usec - start_agg.tv_usec) / 1000000 + (double)(stop_agg.tv_sec - start_agg.tv_sec);
	printf("GPU Time for 1st aggregation: %f sec\n",secs1_agg);
...
//        aggregation_gpu_block<<<gd, bd>>>(device_graph_indexes, device_graph_neighbours, device_feature_data_in_2, device_feature_data_out_2, feature_c.feature_num, feature_c.node_num);
        ...
        cudaDeviceSynchronize();
        gettimeofday(&stop_agg, NULL);
	secs1_agg = (double)(stop_agg.tv_usec - start_agg.tv_usec) / 1000000 + (double)(stop_agg.tv_sec - start_agg.tv_sec);
	printf("GPU Time for 2nd aggregation: %f sec\n",secs1_agg);
	...
	//Timing
        ...
        cudaFree(out_feature);
				...
				float secs1_all = (double)(stop_all.tv_usec - start_all.tv_usec) / 1000000 + (double)(stop_all.tv_sec - start_all.tv_sec);
				printf("GPU Time for overall execution: %f sec\n",secs1_all);
